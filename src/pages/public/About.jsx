    ...
    <div className="min-h-screen relative pt-24 pb-12 overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div ref={headerRef} className="text-center mb-16 opacity-0">
          ...
          <p className="text-xl text-textLight dark:text-gray-300 max-w-3xl mx-auto leading-relaxed">
            {t('aboutDesc')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-24">
          <div ref={missionTextRef} className="glass-panel p-8 opacity-0">
            <h2 className="text-3xl font-bold font-heading text-textDark dark:text-white mb-6">{t('ourMission')}</h2>
            <p className="text-textLight dark:text-gray-400 mb-6 leading-relaxed">
              {t('missionText1')}
            </p>
          ...
          <div ref={missionImageRef} className="relative h-[400px] rounded-2xl overflow-hidden shadow-2xl opacity-0">
            <img 
        ...
        <div ref={featuresRef} className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-24">
          {[
            ...
            <div
              key={index}
              className="feature-item glass-panel p-6 text-center hover:bg-section dark:hover:bg-white/5 transition-colors group opacity-0"
            >
              <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform shadow-md">
                <item.icon size={28} />
              </div>
              <h3 className="text-xl font-bold text-textDark dark:text-white mb-2">{t(item.title)}</h3>
        ...
        <div ref={teamRef} className="text-center opacity-0">
          <h2 className="text-3xl font-bold font-heading text-textDark dark:text-white mb-12">{t('meetOurTeam')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="glass-panel p-6 group">
                <div className="w-32 h-32 mx-auto mb-6 rounded-full overflow-hidden border-2 border-primary/30 shadow-lg">
                  <img src={`https://i.pravatar.cc/150?img=${i + 10}`} alt="Team Member" className="w-full h-full object-cover" />
                </div>
                <h3 className="text-xl font-bold text-textDark dark:text-white mb-1">Alex Morgan</h3>
                <p className="text-primary text-sm mb-4">{t('seniorBroker')}</p>
                <div className="flex justify-center gap-4">
                  {/* Social Icons Placeholder */}
                  <div className="w-8 h-8 rounded-full bg-background dark:bg-white/5 hover:bg-primary hover:text-white transition-colors cursor-pointer flex items-center justify-center">in</div>
                  <div className="w-8 h-8 rounded-full bg-background dark:bg-white/5 hover:bg-primary hover:text-white transition-colors cursor-pointer flex items-center justify-center">tw</div>
                </div>
              </div>
            ))}
          </div>
