...
const DeveloperCard = ({ developer }) => {
  ...
  return (
    <div ref={cardRef} className="h-full stagger-item opacity-0">
      <Link to={`/developers/${developer.id}`} className="block h-full group">
        <div className="h-full bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl overflow-hidden hover:shadow-xl hover:border-primary/30 transition-all duration-300 flex flex-col items-center text-center p-8 relative">
          ...
          <div className="w-24 h-24 rounded-full bg-gray-50 dark:bg-white/10 flex items-center justify-center mb-6 relative z-10 group-hover:scale-110 transition-transform duration-300 shadow-sm border border-gray-100 dark:border-white/5">
            {developer.logo && !developer.logo.includes('placeholder') ? (
              <img src={developer.logo} alt={developer.name} className="w-20 h-20 rounded-full object-cover" />
            ) : (
